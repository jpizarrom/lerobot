...
from lerobot.rl.acfql.buffer_sb3 import ReplayBufferNSteps as ReplayBuffer
...
def test_nstep_early_termination(optimize_memory, done_at, n_steps, base_idx):
    ...
    assert batch["done"].item() == float(base_idx <= done_at and done_at < base_idx + n_steps)
    ...
    if base_idx > done_at:
        ...
        assert batch["next_state"]["observation.image"].shape == (1, 3, 84, 84)
        assert batch["next_state"]["observation.image"].max() == min(done_at + 1, base_idx + n_steps)
        assert batch["next_state"]["observation.state"].shape == (1, 4)
        assert batch["next_state"]["observation.state"].max() == min(done_at + 1, base_idx + n_steps)
...
def test_nstep_early_truncation(optimize_memory, truncated_at, n_steps, base_idx):
    ...
    assert batch["truncated"].item() == float(base_idx <= truncated_at and truncated_at < base_idx + n_steps)
    ...
    if base_idx > truncated_at:
        ...
        assert batch["next_state"]["observation.image"].shape == (1, 3, 84, 84)
        assert batch["next_state"]["observation.image"].max() == min(truncated_at + 1, base_idx + n_steps)
        assert batch["next_state"]["observation.state"].shape == (1, 4)
        assert batch["next_state"]["observation.state"].max() == min(truncated_at + 1, base_idx + n_steps)
...
def test_nstep_no_terminations(optimize_memory, n_steps):
    ...
    torch.testing.assert_close(batch["discount"].item(), gamma**n_steps, rtol=1e-6, atol=1e-6)
    ...
    torch.testing.assert_close(batch["discount"].item(), gamma**2, rtol=1e-6, atol=1e-6)
...
def test_sparse_reward_computation():
    ...
    assert torch.allclose(batch["reward"], torch.tensor([0.9703]), rtol=1e-4, atol=1e-4)
    assert batch["done"].equal(torch.tensor([1], dtype=torch.float32))
    assert batch["truncated"].equal(torch.tensor([0], dtype=torch.float32))
    assert batch["action_is_pad"].equal(torch.tensor([[0, 0, 0, 0]], dtype=torch.bool))
    ...
    batch = buffer.sample_nstep_full_for_indices(torch.tensor([2]), batch_size=1, gamma=0.99, n_steps=4)
    assert torch.allclose(batch["reward"], torch.tensor([0.0]), rtol=1e-4, atol=1e-4)
    assert batch["done"].equal(torch.tensor([0], dtype=torch.float32))
    assert batch["truncated"].equal(torch.tensor([1], dtype=torch.float32))
    assert batch["action_is_pad"].equal(torch.tensor([[0, 0, 0, 0]], dtype=torch.bool))
    ...
    batch = buffer.sample_nstep_full_for_indices(torch.tensor([2]), batch_size=1, gamma=0.99, n_steps=4)
    assert torch.allclose(batch["reward"], torch.tensor([0.9900]), rtol=1e-4, atol=1e-4)
    assert batch["done"].equal(torch.tensor([1], dtype=torch.float32))
    assert batch["truncated"].equal(torch.tensor([0], dtype=torch.float32))
    assert batch["action_is_pad"].equal(torch.tensor([[0, 0, 1, 1]], dtype=torch.bool))
