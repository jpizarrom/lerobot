...
def create_stats_buffers(
    ...
    for key, ft in features.items():
        ...
        if stats:
            if (
                "mean" in stats[key] and isinstance(stats[key]["mean"], np.ndarray)
            ) or ("min" in stats[key] and isinstance(stats[key]["min"], np.ndarray)):
                if norm_mode is NormalizationMode.MEAN_STD:
                ...
                "mean" in stats[key] and isinstance(stats[key]["mean"], torch.Tensor)
            ) or ("min" in stats[key] and isinstance(stats[key]["min"], torch.Tensor)):
